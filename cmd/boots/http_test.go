...
func (c tclient) GetInstanceIDFromIP(_ context.Context, _ net.IP) (string, error) {
	return c.id, c.getErr
}

func (c tclient) PostInstanceEvent(_ context.Context, _ string, _ io.Reader) (string, error) {
	return "", c.postErr
}
...
func TestServeEvents(t *testing.T) {
	for _, test := range []struct {
		...
		resp := w.Result()
		defer resp.Body.Close() //nolint:revive // deferring in a loop
